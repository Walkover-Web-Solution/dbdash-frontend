          ...
          <Box style={{ display: "flex", flexDirection: "column" }}>
            ...
            <Box>
              {query.map((q, index) => (<Box key={index} sx={{ display: "flex", flexDirection: "row" }}>
                ...
                <Box sx={{ mr: 1 }}>
                  <Select value={((AllTableInfo.tables[props?.tableName].filters[props?.filterId].query)==`select * from ${props?.tableName}`)?  fieldData[0]?.id:q?.fields } onChange={(e) => handleChangeField(e, index)} sx={{ width: 150 }}>
                ...
                <Box sx={{ mr: 1 }}>
                  <Select sx={{ width: 150 }}
                    value={((AllTableInfo.tables[props?.tableName].filters[props?.filterId].query)==`select * from ${props?.tableName}`) ?"LIKE":q?.selectedOption }
                    // defaultValue="LIKE"
                    key={q?.value}
                ...
                <Box>
                  ...
                  <TextField required value={((AllTableInfo.tables[props?.tableName].filters[props?.filterId].query)==`select * from ${props?.tableName}`) ?" ":q?.value} sx={{ width: 150, height: 60, fontWeight: 'bold', }} placeholder="Enter the value" type="text" onChange={(e) => handleChangeValue(e, index)} />
