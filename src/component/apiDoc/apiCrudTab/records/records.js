...
function Records(props) {
  ...
  const tableData = async () => {
    const arr = Object.entries(
      props?.alltabledata[props?.table]?.fields
    ).filter(
      (field) =>
        !(
          field[1].fieldType == "rowid" ||
          field[1].fieldType == "autonumber" ||
          field[1].fieldType == "createdat" ||
          field[1].fieldType == "createdby" ||
          field[1].fieldType == "updatedat" ||
          field[1].fieldType == "updatedby"
        )
    );
    setRowFieldData(
      props?.parent != "basicstuff"
        ? arr
        : Object.entries(props?.alltabledata[props?.table]?.fields)
    );
  };
  useEffect(() => {
    tableData();
  ...
  return (
    <div
      className={`recordcontainer ${
        props?.parent == "basicstuff" ? "border-others" : "border-basicstuff"
      }`}
    >
      <Grid container spacing={2}>
        <Grid item xs={props?.CopyButton ? 2 : 3}>
          ...
          {Array.isArray(rowfieldData) &&
            rowfieldData.map((fields, index) =>
              props?.CopyButton ? (
        ...
        {(!props?.parent || props?.parent != "basicstuff") && (
          <>
            ...
            <Grid item xs={3}>
              <Typography
                className="center-aligned  add-remove "
                fontSize={Number(variables.titlesize)}
                fontWeight={variables.titleweight}
                variant={variables.titlevariant}
              >
